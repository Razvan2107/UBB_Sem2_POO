...
map<string,int> ATService::raport()
{
	map<string, int> sol;
	for (auto& a : getAll())
		sol[a.getDestinatie()]++;
...
void TestRaport()
{
	...
	srv.addAT("a", "aad", "i", 200);
	srv.addAT("b", "aad", "f", 300);
	srv.addAT("a", "af", "k", 2500);
	srv.addAT("g", "g", "il", 2030);
	srv.addAT("a", "gf", "mi", 2020);

	map<string, int> test = srv.raport();
	assert(test["aad"] == 2);
}
