...
Agentie_de_Turism ATRepo::find(string denumire, string destinatie)
{
...
vector<Agentie_de_Turism> ATRepo::getAll() const noexcept
{
	return all;
}
...
void ATRepoFile::delete_at(string denumire, string destinatie)
{
	ATRepo::delete_at(denumire, destinatie);
	writeToFile();
}

void ATRepoFile::modify(string denumire, string destinatie, string tip, int pret)
{
	ATRepo::modify(denumire,destinatie,tip,pret);
	writeToFile();
}
...
void TestFileRepo()
{
	...
	test.store(a);
	assert(test.getAll().size() == 4);

	test.delete_at("asd", "zxcv");
	assert(test.getAll().size() == 3);

	test.modify("q", "w", "asdfg", 663);
}
