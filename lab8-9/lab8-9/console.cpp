...
void UI::tip(const vector<Agentie_de_Turism>& agentii,string file)
{
	ofstream fout(file);
	fout << "Agentii de Turism:\n";
	for (const auto& a : agentii)
		fout << a.getDenumire() << ',' << a.getDestinatie() << ',' << a.getTip() << ',' << a.getPret() << '\n';
}
...
void UI::WishListUI()
{
	...
	while (run)
	{
		...
		case 3:
			...
			break;
		case 4:
			tip(srv.getAllWishListService(),"wl.csv");
			break;
		case 5:
			run = 0;
			break;
